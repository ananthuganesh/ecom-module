"""12-digit public customerUrlId helpers (mirror orderUrlId)."""
...
from app.documents import Setting, User

# Random start band for first-ever counter seed (inclusive).
...
async def next_customer_url_id() -> str:
    """Allocate the next 12-digit customerUrlId and advance the settings counter."""
...
async def ensure_customer_url_id(user: User) -> str | None:
    """Assign customerUrlId to a storefront customer if missing. No-op for admins."""
    if getattr(user, "isAdmin", False):
        return None
    existing = getattr(user, "customerUrlId", None)
    if existing and str(existing).isdigit() and len(str(existing)) == 12:
        return str(existing)
    url_id = await next_customer_url_id()
    user.customerUrlId = url_id
    await user.save()
    return url_id
